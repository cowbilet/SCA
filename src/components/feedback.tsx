...
export default function Feedback(data: {status: SubmissionState, mentorNote: string | null, assessorNote: string | null}) {
    ...
    switch (data.status) {
        case "pending assessor":
            if (data.mentorNote) {
                comments.push(
                    <FeedbackNotification key="mentorFeedback" feedback={data.mentorNote} title={"Mentor"} className={positiveClass} />
                )
            }
            break;
        case "rejected mentor":
            if (data.mentorNote) {
                comments.push(
                    <FeedbackNotification key="mentorFeedback" feedback={data.mentorNote} title={"Mentor"} className={negativeClass} />
                )
            }
            break;
        case "rejected assessor":
            if (data.assessorNote) {
                comments.push(
                    <FeedbackNotification key="assessorFeedback" feedback={data.assessorNote} title={"Assessor"} className={negativeClass} />
                )
            }
            if (data.mentorNote) {
                comments.push(
        ...
        case "completed":
             if (data.assessorNote) {
                comments.push(
                    <FeedbackNotification key="assessorFeedback" feedback={data.assessorNote} title={"Assessor"} className={positiveClass} />
                )
            }
            if (data.mentorNote) {
                comments.push(
