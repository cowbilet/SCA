...
export default function InstructionAndFeedback<
    T extends {
...
>({
    ...
    data: T
    Instructions: React.ComponentType<{ status: SubmissionState }>
}): JSX.Element {
    return (
...
function AccordionFeedback({
    ...
    return (
        <div
            className={clsx(
                'border rounded-md border-gray-300 gap-2 pt-2 px-2 flex flex-col bg-white',
                { 'pb-2': isOpen },
            )}
        >
            <div
            ...
            >
                ...
                <p className="text-sm text-gray-500">
                    {isOpen ? 'View Feedback' : 'Hide Feedback'}
                </p>
                <ChevronDown
                    className={clsx(
                        'h-5 w-5 text-gray-500 ml-auto transition-transform',
                        { 'rotate-180': isOpen },
                    )}
                />
            </div>
