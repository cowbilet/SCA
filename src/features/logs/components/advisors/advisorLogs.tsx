...
export function AdvisorLogEntries({
    ...
    const {
        ...
        isPending,
        isError,
    } = useLogs(award, challenge, studentId, type)
    if (isPending) {
...
function MentorLogEntry({ log }: { log: LogEntry }) {
    ...
    const { mutate: approveLog } = useApproveLog(
        log.logId,
        award,
        challenge,
        studentId,
        ...
        <LogEntryScaffold log={log} key={log.logId}>
            <div className="buttons ml-auto flex flex-row items-center gap-1">
                {log.approved === null && (
                    <>
                        <button
                            onClick={() => approveLog({ approved: true })}
                        ...
                        <button
                            onClick={() => approveLog({ approved: false })}
                            className=" text-red-500 font-bold rounded w-5 h-5 flex items-center justify-center hover:cursor-pointer"
                        >
                            <X />
                        </button>
                    </>
