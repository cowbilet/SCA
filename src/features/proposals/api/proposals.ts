...
import { createDbChallengeProposal } from '@/db/proposals'
...
const inputSchema = CreateProposalSchema.extend({
...
export const createUserProposal = createServerFn({ method: 'POST' }).inputValidator(inputSchema).handler(async ({data}): Promise<void> => {
